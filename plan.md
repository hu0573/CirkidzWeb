...
保持单层级左侧导航栏（Sidebar），包含以下四个核心页面，顶部再配一个极简 Header（显示当前用户和一个占位按钮）：

1. **Dashboard** – 摘要总览
2. **Free Trials** – 免费体验课报名管理
3. **Enrolments** – 正式付费学员管理
4. **Class Scheduling** – 简化课程排班视图
...
实施两个即可：
1. `StatusBadge` – 根据状态名称映射颜色，覆盖 Free Trials 与 Enrolments。
2. `EntityDrawer` – 右侧抽屉展示学生/报名详情，接受通用 props（标题、字段数组、备注文本）。
